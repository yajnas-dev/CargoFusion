...
- `simulation/` — `DemoControls` (one-shot operator actions) and `SimulationEngine` (opt-in interval-driven background drift). Never auto-starts; only starts via an explicit API call, so it can't interfere with tests or an idle dev session.
