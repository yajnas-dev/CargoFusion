...
| `Incident` | A reported equipment-offline or lane-blocked disruption with a cause, `reportedBy`, `startedAt`, `resolvedAt`/`resolvedBy` — the lifecycle record that `Equipment.status`/`YardLane.blocked` alone don't carry. Report/resolve via `IncidentService` (`src/incidents/`), `/incidents`. |
...
A lifecycle record layered on top of the state mutation `DemoControls` already performs — `report()` calls `setEquipmentStatus(OFFLINE)`/`blockLane()` and creates an `Incident` row (cause, reporter, start time); `resolve()` calls the inverse mutation and stamps `resolvedAt`/`resolvedBy`. Guards against reporting a duplicate open incident for the same subject, and against reporting equipment that isn't currently `AVAILABLE`. Every report/resolve writes an `AuditEvent` (`INCIDENT_REPORTED`/`INCIDENT_RESOLVED`).

## What-If Preview (`src/whatif/WhatIfService.ts`)

Read-only "what would this affect" checks, reusing the exact same logic the real detectors use rather than a separate simulation:

- **Lane block preview** — builds a `YardGraph` from a locally patched copy of the lane list (never written to the database) and checks every active task's recorded route against it, same as `blockedLaneImpact`'s reroute logic.
